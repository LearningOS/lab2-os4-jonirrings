...
impl MemorySet {
    ...
    pub fn map_create(&mut self, start: VirtAddr, len: usize, perm: MapPermission) -> isize {
    ...
    pub fn map_destroy(&mut self, start: VirtAddr, len: usize) -> isize {
        let end = VirtAddr(start.0 + len);
        let vr = VPNRange::new(start.floor(), end.ceil());
...
// mapping area, manages mapping ifo
// for direct data, which means not mapped, does not need be in the area
// but should be in the page_table, who manages which memory is in used
impl MapArea {
    pub fn new(
        start_va: VirtAddr,
        end_va: VirtAddr,
