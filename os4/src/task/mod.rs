...
impl TaskManager {
    ...
    fn translate_current_va(&self, va: VirtAddr) -> Option<PhysAddr> {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].memory_set.translate_addr(va)
    }
    fn current_mem_map(&self, start: VirtAddr, len: usize, perm: MapPermission) -> isize {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].memory_set.map_create(start, len, perm)
    }
    fn current_mem_unmap(&self, start: VirtAddr, len: usize) -> isize {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].memory_set.map_destroy(start, len)
    }
...
/// Translate current va to pa
pub fn translate(va: VirtAddr) -> Option<PhysAddr> {
    TASK_MANAGER.translate_current_va(va)
}

pub fn current_m_map(start: VirtAddr, len: usize, perm: MapPermission) -> isize {
    TASK_MANAGER.current_mem_map(start, len, perm)
}

pub fn current_m_unmap(start: VirtAddr, len: usize) -> isize {
    TASK_MANAGER.current_mem_unmap(start, len)
}
